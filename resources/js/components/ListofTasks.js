      ...
      <List component="nav">
        {tasks.map(task => (
          <ListItem key={task.id} >
            {task.title} { task.is_completed ? ' - Task is completed' : ' - Task is not completed' }
            <ListItemSecondaryAction>
              <Tooltip disabled={task.is_completed} title="Mark as completed" aria-label="x" size="small" disa onClick={this.handleMarkTaskAsCompleted.bind(this, task.id)}>
